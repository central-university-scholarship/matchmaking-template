    ...
    for num in range(20):
        ...
        while (True):
            ...
            response = requests.get(
                f'http://server:8000/matchmaking/users?test_name=test_{num}&epoch={new_epoch}')
            ...
            for i in range(len(users)):
                ...
                candidates_uids.append(users[i].user_id)
            ...
            top_candidates.sort(key=lambda x: x.rating, reverse=True)
            # for x in top_candidates:
            #     print(x.rating)
            mid_candidates.sort(key=lambda x: x.rating, reverse=True)
            bot_candidates.sort(key=lambda x: x.rating, reverse=True)
            sup_candidates.sort(key=lambda x: x.rating, reverse=True)
            jungle_candidates.sort(key=lambda x: x.rating, reverse=True)
            ...
            while (True):
                ...
                '''
                print('Red:')
                p = [red_top_player.uid, red_mid_player.uid, red_bot_player.uid, red_sup_player.uid, red_jungle_player.uid]
                for x in users:
                    if (any(x.user_id == i for i in p)):
                        print(x.mmr)
                print('Blue:')
                p = [blue_top_player.uid, blue_mid_player.uid, blue_bot_player.uid, blue_sup_player.uid, blue_jungle_player.uid]
                for x in users:
                    if (any(x.user_id == i for i in p)):
                        print(x.mmr)
                '''
                ...
                if (len(top_candidates) >= 2 and len(mid_candidates) >= 2 and len(bot_candidates) >= 2 and len(
                        sup_candidates) >= 2 and len(jungle_candidates) >= 2):
                    candidates_uids.remove(red_top_player.uid)
                    candidates_uids.remove(red_mid_player.uid)
                    candidates_uids.remove(red_bot_player.uid)
                    candidates_uids.remove(red_sup_player.uid)
                    candidates_uids.remove(red_jungle_player.uid)
                    candidates_uids.remove(blue_top_player.uid)
                    candidates_uids.remove(blue_mid_player.uid)
                    candidates_uids.remove(blue_bot_player.uid)
                    candidates_uids.remove(blue_sup_player.uid)
                    candidates_uids.remove(blue_jungle_player.uid)
                else:
                    break
            if (len(candidates_uids)>0):
                add_users = []
                for x in users:
                    if (any(x.user_id == uid for uid in candidates_uids)):
                        add_users.append(x)
            ...
            response = requests.post(
                f'http://server:8000/matchmaking/match?test_name=test_{num}&epoch={new_epoch}',
                headers=headers,
                json={"example": data1})
            logger.info(response.text)
            ne = response.json()
            new_epoch = ne['epoch']
            #print(new_epoch)
            if (new_epoch == '00000000-0000-0000-0000-000000000000'):
                break